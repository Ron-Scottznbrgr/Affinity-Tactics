...
r_tile = round(random_range(2, 5));
switch(r_tile){
	
case 2: c_tile = round(random_range(4, 5));
break;
case 3: c_tile = round(random_range(4, 6));
break;
case 4: c_tile = round(random_range(4, 6));
break;
case 5: c_tile = round(random_range(5, 6));
break;
}
}
else{
////////////////////////////////////If we are creating enemies...
r_tile = round(random_range(0, 3));
switch(r_tile){
case 0: c_tile = round(random_range(0, 1));
break;
case 1: c_tile = round(random_range(0, 2));
break;
case 2: c_tile = round(random_range(0, 2));
break;
case 3: c_tile = round(random_range(1, 2));
break;
}
}

}
until (global.hex_tile_array[c_tile,r_tile].occupied==false);
}
...
with (instance_create_layer(global.hex_tile_array[c_tile,r_tile].x,global.hex_tile_array[c_tile,r_tile].y , "Instances", obj_char)){
...
{
	//GenEnemy(Enemy#,Array#,<Stats> or Spells)
...
//Assigning Spells to Character. Should be 18 Slots.
for (var xxy = 0; xxy < array_length_1d(obj_OW_Player.H_Fire_Spells); xxy += 1) {
	
if (i==0){
char_spell[xxy]=obj_OW_Player.H_Fire_Spells[xxy];

//show_debug_message(obj_OW_Player.H_Fire[0])

}
else if (i==1){
char_spell[xxy]=obj_OW_Player.H_Water_Spells[xxy];
	//show_debug_message(obj_OW_Player.H_Water[0])
}
else if (i==2){
char_spell[xxy]=obj_OW_Player.H_Earth_Spells[xxy];
	//show_debug_message(obj_OW_Player.H_Earth[0])
}
else if (i==3){
char_spell[xxy]=obj_OW_Player.H_Air_Spells[xxy];
	//show_debug_message(obj_OW_Player.H_Air[0])
}
else
{
	//GenEnemy(Enemy#,Array#,Stats or <Spells>)
	char_spell[xxy]= (Battle_Generate_Enemy(obj_OW_Player.enemy_data[i-4],xxy,1));
}
}
...
ds_grid_set(global.turnOrder,0,i, char_data[1]);//Sprite
ds_grid_set(global.turnOrder,1,i, char_data[7]);//Speed
ds_grid_set(global.turnOrder,2,i, char_data[0]);//Name
ds_grid_set(global.turnOrder,3,i, id);//ID
ds_grid_set(global.turnOrder,4,i, char_data[7]);//ClockTicks-Initilized to speed


event_perform_object(obj_char,ev_alarm, 11);
char_has_data=true;

char_tile_number=global.hex_tile_array[c_tile,r_tile].id;
char_tile_x=global.hex_tile_array[c_tile,r_tile].array_x;
char_tile_y=global.hex_tile_array[c_tile,r_tile].array_y;
global.hex_tile_array[c_tile,r_tile].occupied=true;
